...
import { EtherCatDataKey, EtherCatDeviceConfig } from '../../../models/public-api';
import { EtherCatDeviceDialogComponent, EtherCatDeviceDialogData } from '../ethercat-device-dialog/ethercat-device-dialog.component';
import {
  EtherCatTagImportDialogComponent,
  EtherCatTagImportDialogData,
  EtherCatTagImportResult,
} from '../ethercat-tag-import-dialog/ethercat-tag-import-dialog.component';

@Component({
  selector: 'tb-ethercat-devices-table',
  templateUrl: './ethercat-devices-table.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => EtherCatDevicesTableComponent),
      multi: true,
    },
    {
...
export class EtherCatDevicesTableComponent implements ControlValueAccessor, Validator {

  @Input() gatewayDeviceId: string;
  @Input() connectorName: string;

  readonly displayedColumns = ['deviceName', 'slave', 'tags', 'actions'];

  devices: EtherCatDeviceConfig[] = [];
  filteredDevices: EtherCatDeviceConfig[] = [];
  ...
  private onChange: (value: EtherCatDeviceConfig[]) => void;

  writeValue(devices: EtherCatDeviceConfig[]): void {
    this.devices = devices || [];
    this.updateFilter();
    this.cd.markForCheck();
  }

  registerOnChange(fn: (value: EtherCatDeviceConfig[]) => void): void {
    this.onChange = fn;
  }
  ...
  getTagCount(device: EtherCatDeviceConfig): number {
  ...
  getOriginalIndex(device: EtherCatDeviceConfig): number {
  ...
  addDevice(): void {
    this.dialog.open<EtherCatDeviceDialogComponent, EtherCatDeviceDialogData, EtherCatDeviceConfig>(
      EtherCatDeviceDialogComponent, {
        data: {
          isEdit: false,
  ...
  exportTags(index: number): void {
    ...
    const addKeys = (keys: EtherCatDataKey[], category: string) => {
      for (const k of (keys || [])) {
    ...
    a.href = url;
    a.download = `${device.deviceName || 'ethercat-device'}-tags.csv`;
    a.click();
  ...
  importTags(index: number): void {
    ...
    const existingTags: EtherCatDataKey[] = [
      ...(device.timeseries || []),
      ...(device.attributes || []),
      ...(device.attributeUpdates || []),
    ];

    this.dialog.open<EtherCatTagImportDialogComponent, EtherCatTagImportDialogData, EtherCatTagImportResult>(
      EtherCatTagImportDialogComponent, {
