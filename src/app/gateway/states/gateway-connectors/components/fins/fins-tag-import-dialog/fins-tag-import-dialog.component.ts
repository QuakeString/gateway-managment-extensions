...
const VALUE_TYPE_MAP: Record<string, FinsValueType> = {
  'bool': FinsValueType.BOOL,
  'boolean': FinsValueType.BOOL,
  'bit': FinsValueType.BOOL,
  'binary': FinsValueType.BOOL,
  'word': FinsValueType.UINT16,
  'channel': FinsValueType.UINT16,
  'uint': FinsValueType.UINT16,
  'uint16': FinsValueType.UINT16,
  'int': FinsValueType.INT16,
  'int16': FinsValueType.INT16,
  'dword': FinsValueType.UINT32,
  'udint': FinsValueType.UINT32,
  'uint32': FinsValueType.UINT32,
  'dint': FinsValueType.INT32,
  'int32': FinsValueType.INT32,
  'lword': FinsValueType.UINT64,
  'ulint': FinsValueType.UINT64,
  'uint64': FinsValueType.UINT64,
  'lint': FinsValueType.INT64,
  'int64': FinsValueType.INT64,
  'real': FinsValueType.FLOAT32,
  'float': FinsValueType.FLOAT32,
  'float32': FinsValueType.FLOAT32,
  'lreal': FinsValueType.FLOAT64,
  'double': FinsValueType.FLOAT64,
  'float64': FinsValueType.FLOAT64,
  'string': FinsValueType.STRING,
  'wstring': FinsValueType.STRING,
};

export interface FinsTagImportDialogData {
  deviceName: string;
  existingTags: FinsDataKey[];
}

export interface FinsTagImportResult {
...
type FinsCategory = 'timeseries' | 'attributes' | 'attributeUpdates' | 'rpc';

interface ParsedTag {
  tag: string;
  address: string;
  valueType: FinsValueType | null;
  multiplier: number | null;
  divider: number | null;
  reportStrategy: ReportStrategyConfig | null;
  category: FinsCategory;
...
@Component({
  selector: 'tb-fins-tag-import-dialog',
  templateUrl: './fins-tag-import-dialog.component.html',
  styleUrls: ['./fins-tag-import-dialog.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  standalone: true,
  imports: [CommonModule, SharedModule, ReactiveFormsModule],
})
export class FinsTagImportDialogComponent {
  ...
  constructor(
    private dialogRef: MatDialogRef<FinsTagImportDialogComponent>,
    private cd: ChangeDetectorRef,
    private translate: TranslateService,
    @Inject(MAT_DIALOG_DATA) public data: FinsTagImportDialogData,
  ) {
  ...
  importTags(): void {
    const result: FinsTagImportResult = {
      timeseries: [],
      attributes: [],
      attributeUpdates: [],
      rpc: [],
    ...
    for (const tag of this.validTags) {
      ...
      // so build an FinsRpcConfig instead of a data key.
      if (tag.category === 'rpc') {
        const rpc: FinsRpcConfig = {
          method: tag.tag,
          address: tag.address,
  ...
  private processRows(): void {
    ...
    for (const row of this.rawRows) {
      ...
      // Validate — the address must be a CX-Programmer-style FINS address;
      ...
      } else if (!isValidFinsAddress(rawAddress)) {
        errors.push(this.translate.instant('gateway.fins-invalid-address'));
      } else if (isFinsBitAddress(rawAddress) && valueType && valueType !== FinsValueType.BOOL) {
  ...
  private resolveValueType(raw: string): FinsValueType | null {
    if (!raw) { return null; }
  ...
  private normalizeCategory(raw: string): FinsCategory {
