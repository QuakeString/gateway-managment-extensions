...
import { FinsDataKey, FinsDeviceConfig } from '../../../models/public-api';
import { FinsDeviceDialogComponent, FinsDeviceDialogData } from '../fins-device-dialog/fins-device-dialog.component';
import {
  FinsTagImportDialogComponent,
  FinsTagImportDialogData,
  FinsTagImportResult,
} from '../fins-tag-import-dialog/fins-tag-import-dialog.component';

@Component({
  selector: 'tb-fins-devices-table',
  templateUrl: './fins-devices-table.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => FinsDevicesTableComponent),
      multi: true,
    },
    {
...
export class FinsDevicesTableComponent implements ControlValueAccessor, Validator {
  ...
  filteredDevices: FinsDeviceConfig[] = [];
  ...
  private onChange: (value: FinsDeviceConfig[]) => void;

  writeValue(devices: FinsDeviceConfig[]): void {
    this.devices = devices || [];
    this.updateFilter();
    this.cd.markForCheck();
  }

  registerOnChange(fn: (value: FinsDeviceConfig[]) => void): void {
    this.onChange = fn;
  }
  ...
  getTagCount(device: FinsDeviceConfig): number {
  ...
  getOriginalIndex(device: FinsDeviceConfig): number {
  ...
  addDevice(): void {
    this.dialog.open<FinsDeviceDialogComponent, FinsDeviceDialogData, FinsDeviceConfig>(
      FinsDeviceDialogComponent, {
        data: {
          isEdit: false,
  ...
  exportTags(index: number): void {
    ...
    const addKeys = (keys: FinsDataKey[], category: string) => {
      for (const k of (keys || [])) {
    ...
    a.href = url;
    a.download = `${device.deviceName || 'fins-device'}-tags.csv`;
    a.click();
  ...
  importTags(index: number): void {
    ...
    const existingTags: FinsDataKey[] = [
      ...(device.timeseries || []),
      ...(device.attributes || []),
      ...(device.attributeUpdates || []),
    ];

    this.dialog.open<FinsTagImportDialogComponent, FinsTagImportDialogData, FinsTagImportResult>(
      FinsTagImportDialogComponent, {
